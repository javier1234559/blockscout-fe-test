...
const IGNORED_ENVS = ["NEXT_PUBLIC_GIT_COMMIT_SHA", "NEXT_PUBLIC_GIT_TAG"];
...
function updateFileContent(
  envsEntries: Array<[string, string]>,
  ...
  const chainName =
    envsEntries.find(([key]) => key === "NEXT_PUBLIC_NETWORK_NAME")?.[1] ??
    "Unknown";
  ...
  fs.writeFileSync(
    path.resolve(__dirname, `../../configs/envs/.env.${presetId}`),
    content
  );
}
...
async function updatePresetFile(presetId: keyof typeof PRESETS) {
  ...
  const instanceConfig = (await response.json()) as Record<
    "envs",
  ...
  const presetEnvsEntries = Object.entries(instanceConfig.envs).filter(
    ([key]) => !ignoredEnvs.includes(key)
  );
