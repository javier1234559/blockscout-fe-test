...
const SearchBar = ({ isHomepage, dictionary }: Props) => {
  ...
  const { searchTerm, debouncedSearchTerm, handleSearchTermChange, query, pathname } =
    useQuickSearchQuery()
  ...
  const handleSubmit = React.useCallback(
    (event: FormEvent<HTMLFormElement>) => {
      ...
      if (searchTerm) {
        ...
        router.push({ pathname: '/search-results', query: { q: searchTerm } }, undefined, {
          shallow: true,
        })
      }
    },
  ...
  return (
    <>
      ...
      >
        ...
        <Portal>
          <PopoverContent w={`${menuWidth.current}px`} ref={menuRef}>
