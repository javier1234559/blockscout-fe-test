...
const SearchBarSuggest = ({
  ...
  const handleScroll = React.useCallback(() => {
    ...
    if (
      categoriesRefs.current[
        categoriesRefs.current.length - 1
      ].getBoundingClientRect().y <= topLimit
    ) {
  ...
  const itemsGroups = React.useMemo(() => {
    ...
    if (
      ...
      !map.block &&
      regexp.BLOCK_HEIGHT.test(searchTerm)
    ) {
      map["block"] = [
        {
          type: "block",
  ...
  const content = (() => {
    if (query.isPending || marketplaceApps.isPlaceholderData) {
      return (
        <ContentLoader text="We are searching, please wait... " fontSize="sm" />
      );
    }
              ...
              <TabList columnGap={3} rowGap={2} flexWrap="wrap">
                {resultCategories.map((cat, index) => (
                  <Tab
                    ...
                    {...(tabIndex === index ? { "data-selected": "true" } : {})}
                  >
                    {cat.title}
                  </Tab>
                ))}
