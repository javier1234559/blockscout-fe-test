...
const VerifiedContracts = () => {
  ...
  const [searchTerm, setSearchTerm] = React.useState(
    getQueryParamString(router.query.q) || undefined
  );
  const [type, setType] = React.useState(
    (getQueryParamString(
      router.query.filter
    ) as VerifiedContractsFilters["filter"]) || undefined
  );
  const [sort, setSort] = React.useState<
    VerifiedContractsSortingValue | undefined
  >(
    getSortValueFromQuery<VerifiedContractsSortingValue>(
      router.query,
      SORT_OPTIONS
    )
  );
  ...
  const {
    ...
    sorting: getSortParamsFromValue<
      VerifiedContractsSortingValue,
      VerifiedContractsSortingField,
      VerifiedContractsSorting["order"]
    >(sort),
    options: {
  ...
  const handleTypeChange = React.useCallback(
    (value: string | Array<string>) => {
      ...
      const filter =
        value === "all"
          ? undefined
          : (value as VerifiedContractsFilters["filter"]);
  ...
  const actionBar = (
    <>
      ...
      {(!isMobile || pagination.isVisible) && (
        <ActionBar my={6} position="relative">
          <HStack spacing={3} display={{ base: "none", lg: "flex" }} w="100%">
            {filterInput}
            {sortButton}
            {typeFilter}
          </HStack>
  ...
  return (
    <Box>
      <PageTitle
        title={
          config.meta.seo.enhancedDataEnabled
            ? `Verified ${config.chain.name} contracts`
            : "Verified contracts"
        }
        withTextAd
      />
      <Container maxW="container.xl" mb={6} p={0}>
      ...
      <Container maxW="container.xl">
        <Box mt="1rem" my={8}>
          <Pagination ml="auto" w="fit-content" {...pagination} />
        </Box>
      </Container>
    </Box>
  );
