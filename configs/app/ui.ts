...
const hiddenLinks = (() => {
  const parsedValue =
    parseEnvJson<Array<NavigationLinkId>>(getEnvValue('NEXT_PUBLIC_NAVIGATION_HIDDEN_LINKS')) || []

  if (!Array.isArray(parsedValue)) {
    return undefined
  }

  const result = NAVIGATION_LINK_IDS.reduce(
    (result, item) => {
...
const HOMEPAGE_PLATE_BACKGROUND_DEFAULT = 'linear-gradient(90deg, #F40993 0%, #3500CB 100%);'
...
const UI = Object.freeze({
  ...
  homepage: {
    ...
    plate: {
      background:
        getEnvValue('NEXT_PUBLIC_HOMEPAGE_PLATE_BACKGROUND') || HOMEPAGE_PLATE_BACKGROUND_DEFAULT,
      textColor: getEnvValue('NEXT_PUBLIC_HOMEPAGE_PLATE_TEXT_COLOR') || 'white',
    },
    showAvgBlockTime:
      getEnvValue('NEXT_PUBLIC_HOMEPAGE_SHOW_AVG_BLOCK_TIME') === 'false' ? false : true,
  },
  views,
  indexingAlert: {
    blocks: {
  ...
  ides: {
    items:
      parseEnvJson<Array<ContractCodeIde>>(getEnvValue('NEXT_PUBLIC_CONTRACT_CODE_IDES')) || [],
  },
  hasContractAuditReports:
    getEnvValue('NEXT_PUBLIC_HAS_CONTRACT_AUDIT_REPORTS') === 'true' ? true : false,
  colorTheme: {
