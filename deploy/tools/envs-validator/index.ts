...
async function checkPlaceholdersCongruity(envsMap: Record<string, string>) {
  try {
    ...
    const runTimeEnvs = await getEnvsPlaceholders(
      path.resolve(__dirname, ".env.registry")
    );
    const buildTimeEnvs = await getEnvsPlaceholders(
      path.resolve(__dirname, ".env")
    );
    const envs = Object.keys(envsMap).filter(
      (env) => !buildTimeEnvs.includes(env)
    );
